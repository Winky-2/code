...
# --- 輸出 ---
OUTPUT_XLSX = "B3_mask輔助對照_11_batch4.xlsx"
VIS_DIR = "B3_mask視覺化"
...
def project_len(pA, pB, centroid, axis):
    ...
    tA = float((np.asarray(pA, dtype=np.float64) - centroid) @ axis)
    tB = float((np.asarray(pB, dtype=np.float64) - centroid) @ axis)
    return abs(tA - tB)
...
def load_gt_target_bbox(label_path: Path, width: int, height: int):
    """從 pose 格式的 GT label 撈出「目標牙」的 bbox 角點座標(像素空間)。
    ...
    with open(label_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if v1 == 2 and v2 == 2:
                ...
                return yolo_to_corners(cx * width, cy * height, w * width, h * height)
    return None
...
def draw(img_path, chosen, others, pA, pB, end1, end2, out_path):
    """黃色=選中的 mask，灰色=其他候選 mask，橘線=長軸幾何端點，
    綠/紅=pose 預測的 A/B 點。翻圖時只要確認黃色輪廓套在綠紅兩點那顆牙上。"""
...
def main():
    ...
    for _, r in df_kp.iterrows():
        ...
        img = cv2.imread(str(img_path))
        H, W = img.shape[:2]
        gt_bbox = load_gt_target_bbox(gt_dir / f"{stem}.txt", W, H) if gt_dir else None
    ...
    gt_col = "真實AB像素長度_letterbox px"
    if B2_XLSX and Path(B2_XLSX).exists():
        df_b2 = pd.read_excel(B2_XLSX, sheet_name=B2_SHEET)
        if gt_col in df_b2.columns:
            df = df.merge(df_b2[["圖片檔名", gt_col]], on="圖片檔名", how="left")
            items = []
            for name, col in (("原始預測", "AB長度_原始預測_letterbox px"),
                              ("長軸投影", "AB長度_長軸投影_letterbox px"),
                              ("mask幾何", "AB長度_mask幾何_letterbox px")):
                if col not in df.columns:
                    continue
                d = (df[col] - df[gt_col]).dropna()
                if d.empty:
                    continue
                items.append({
                    "方法": name,
