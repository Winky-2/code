function C1_pixelToMmPredictor_fullimage()
%% C1_pixelToMmPredictor.m
% ============================================================
% 對應流程圖 C 區塊：「轉換實際長度(-0.5mm)」+「臨床與安全性評估
% (MAE/理想比率/過長率)」。
%
% *** 跟舊版 A_pixelToMmPredictor_fixed_v2.m 的關係 ***
% 這支的核心邏輯(ANN迴歸、offset校正、理想比率/過長率計算)完全沿用
% 舊版，沒有變動——因為這部分本來就跟「pixel長度是怎麼算出來的」
% (裁切+letterbox兩階段 vs 全片單階段)無關，只要餵進來的Excel格式
% 一樣(圖片檔名/填充物長度(mm)/像素長度/資料夾來源)，這支就能直接跑。
% 主要差異是檔名(_fullimage後綴)跟下面這段「資料來源」的說明更新。
%
% *** 極重要：實際mm長度只有33張test圖有，這決定了train從哪裡來 ***
% 醫師記錄的「填充物長度(mm)」(根管充填長度_20260826.xlsx)只涵蓋
% 加星號的33顆target teeth，2b訓練pose模型用的其他一般牙齒完全沒有
% 對應的實際mm長度可查——沒有Y就沒辦法訓練/驗證ANN，所以這份合併
% Excel裡「每一列」追根究底都只能來自這33顆牙本身(可能含它們的
% 擴增版本)，不會有其他牙齒混進來。
%
% 在這個前提下，SPLIT_COLUMN('資料夾來源')的train/test意義是：
%
%   資料夾來源 = "train" 的列：
%       這33顆牙裡「有被拿去訓練 2b_train_yolo_pose_fullimage.py」
%       的那個子集(連同它們的Roboflow擴增版本_aug1~_aug6)。因為
%       這些牙本來就是pose模型的訓練標註資料，A/B點是人工標註的
%       ground truth，pixel長度要直接用GT座標算
%       (sqrt((xA-xB)^2+(yA-yB)^2))，不要用模型推論——這樣訓練ANN
%       用的是乾淨訊號，学的是「pixel長度->mm」真正的映射關係，
%       不會混進pose模型自己的預測誤差。
%
%   資料夾來源 = "test" 的列：
%       這33顆牙裡「完全沒有被2b訓練用到、真正held-out」的子集，
%       未經過任何擴增，pixel長度必須來自
%       2c_test_pose_and_measure_fullimage.py 輸出
%       (yolo像素預測_全片版.xlsx 裡的「AB像素長度_原圖px」欄)，
%       也就是「模型預測」的結果，不能偷用GT——這裡要驗證的是
%       pose模型預測A/B點 -> ANN換算成mm，整條pipeline端到端的
%       真實誤差，如果用GT算test的pixel長度，test指標會失真地
%       偏樂觀，不能反映實際部署會遇到的狀況。
%
%   如果這兩邊搞反了(train用模型預測、test用GT)，數字會完全沒有
%   參考價值：train端會混進不必要的雜訊、test端會失去「驗證端到端
%   準確度」的意義。
%
%   ⚠️ 樣本量提醒：整個ANN能用的資料母體就是這33顆牙(+擴增版本)，
%   量本來就偏少(這也是memory裡提到HIDDEN_SIZE=5、trainbr這種
%   保守選擇的原因)，train/test切分後test可能只剩個位數到十幾筆，
%   理想比率/過長率這類百分比指標一筆對錯就大幅跳動，解讀時務必
%   留意樣本數，不要對單一數字過度解讀。
% ============================================================
    ...
    %% --- 2. 欄位適應與清理 ---
    toothIDs = data.('圖片檔名');
    actualLengths = data.('填充物長度(mm)');
    pixelLengths = data.('像素長度');
    ...
    %% 4. 沿用合併腳本已經決定好的 train/test 分組(不重新隨機切分)
    if ~ismember(SPLIT_COLUMN, data.Properties.VariableNames)
        error(['錯誤：Excel裡找不到 "', SPLIT_COLUMN, '" 欄位。請確認合併腳本 ', ...
            '已經有加上「資料夾來源」欄位，並且train用GT算、test用', ...
            '2c_test_pose_and_measure_fullimage.py的模型預測結果算。']);
    end
    ...
    fprintf('=== 資料切分(沿用合併腳本原始分組，非重新隨機切分)：train %d 筆 (%.0f%%) / test %d 筆 (%.0f%%) ===\n', ...
    ...
    % 注意：這裡的test pixel長度必須來自2c(模型推論)，見檔頭說明，
    % 否則這個「唯一嚴格指標」不能反映真實部署誤差。
    Y_test  = Y(testIdx);
    ...
    MetricNames = {
        sprintf('A_Test set表現 (held-out %d筆，pixel長度來自模型預測，唯一一次的嚴格指標)', n_test);
    ...
    %% 9. 匯出至 Excel 檔案
    output_filename = '預測結果與評估指標_train80_test20_像素轉mm_全片版.xlsx';
    ...
    fprintf('\n=== 臨床與安全性評估 (對應流程圖 C 區塊最後一步) ===\n');
    fprintf('offset = -%.2f mm，理想誤差容忍 = ±%.2f mm（兩者皆為預設值，需醫師確認後調整）\n', OFFSET_MM, IDEAL_TOLERANCE_MM);
    fprintf('[Test set，唯一嚴格指標，pixel長度來自模型預測]  MAE(校正後): %.3f mm | 理想比率: %.1f%% | 過長率: %.1f%%\n', ...
    ...
    fprintf('👉 接下來請執行 C2_plotMyResults_fullimage.m 產生圖表\n');
