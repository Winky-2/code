...
from image_enhance import enhance_image  # 跟B2共用同一份CLAHE增強邏輯，train/test參數保證一致

# ---------------- 設定區 ----------------
POSE_DATA_DIR = "132-train-second-model_v2"    # 👈 Roboflow匯出：完整X光片，單一class + keypoint visibility標目標牙
WORK_DIR = "yolo11_pose_run-batch4_前處理"              # 訓練用資料/權重輸出的工作資料夾
...
def copy_items(items, dst_img_dir, dst_lbl_dir):
    """圖片這裡改成讀取->CLAHE增強->寫出(不再是單純複製檔案)，
    label直接複製，因為增強只動pixel intensity、不動geometry，
    YOLO-pose的bbox/keypoint座標標註完全不受影響。
    這裡套用的enhance_image()跟B2推論時用的是同一份程式碼、同一組
    參數(image_enhance.py)，避免train/test分布不一致。"""
    dst_img_dir.mkdir(parents=True, exist_ok=True)
    dst_lbl_dir.mkdir(parents=True, exist_ok=True)
    for item in items:
        img = cv2.imread(str(item["img_path"]))
        if img is None:
            print(f"⚠️ 讀不到 {item['img_path']}，改用原始複製(未增強)")
            shutil.copy2(item["img_path"], dst_img_dir / item["img_path"].name)
        else:
            img_enhanced = enhance_image(img)
            cv2.imwrite(str(dst_img_dir / item["img_path"].name), img_enhanced)
