...
# ---------------- 設定區 ----------------
POSE_WEIGHTS = "yolo11_pose_run-batch4_重新標註-adjust/weights_ready.pt"   # 👈 2b_..._fullimage.py訓練完成的權重

# 要跑推論的圖片資料夾：完整X光片test圖(不是裁切/letterbox後的圖)，
# 通常就跟GT_IMAGE_DIR是同一份
IMAGE_DIR = "33-all-test_enhanced1280/test/images"

# 舊標註(ground truth)：這支必須要有，因為要靠它的bbox做IoU比對來認牙，
# 不是只拿來算誤差而已，所以跟兩階段版不同，這裡不支援設成None
GT_LABEL_DIR = "33-all-test_enhanced1280/test/labels"
GT_IMAGE_DIR = "33-all-test_enhanced1280/test/images"

OUTPUT_XLSX = "yolo像素預測_11_batch4_重新標註-adjust.xlsx"
RAW_KEYPOINT_CSV = "yolo關鍵點原始座標_11_batch4_重新標註-adjust.csv"
VIS_DIR = "pose_預測視覺化_11_batch4_重新標註-adjust"  # 推論結果視覺化輸出資料夾
SAVE_VISUALIZATION = True

IMG_SIZE = 1280               # 👈 要跟2b_..._fullimage.py訓練時的IMG_SIZE一致
