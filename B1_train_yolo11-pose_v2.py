...
# ---------------- 設定區 ----------------
POSE_DATA_DIR = "132-train-second-model_enhanced960"    # 👈 Roboflow匯出：完整X光片，單一class + keypoint visibility標目標牙
WORK_DIR = "yolo11_pose_run-batch4_重新標註-adjust960"              # 訓練用資料/權重輸出的工作資料夾
...
def count_bboxes_per_image(items):
    """算items(通常傳train_items)裡每一張圖片的label檔案有幾行(=幾個
    bbox/牙齒instance)，並拆分「目標牙(A/B都v=2)」跟「一般牙(v=0)」的
    數量。這跟pose mAP無關，純粹是資料量統計，方便確認：
      - 每張圖平均標了幾顆牙(bbox總數/張數)
      - 一般牙(負樣本)的標註密度夠不夠，太稀疏可能讓keypoint head
        對「這裡沒有目標牙」學得不夠。
    回傳一個list[dict]，每個dict對應一張圖，方便直接組成DataFrame。"""
    records = []
    for item in items:
        ...
        for line in lines:
            parts = line.split()
            if len(parts) < 11:
                continue  # 格式不完整的行(理論上不該出現)，不計入統計
            v1 = float(parts[7])
            v2 = float(parts[10])
            if v1 == 2 and v2 == 2:
                n_target += 1
...
def train_pose_model():
    ...
    # train集每張圖的bbox(牙齒instance)數量統計，跟pose mAP無關，
    # 純粹是資料量紀錄，不管有沒有讀到val_metrics都要寫進Excel。
    bbox_records = count_bboxes_per_image(train_items)
    bbox_df = pd.DataFrame(bbox_records)
    total_bbox = int(bbox_df["bbox總數"].sum()) if not bbox_df.empty else 0
    total_target = int(bbox_df["目標牙數量(v=2)"].sum()) if not bbox_df.empty else 0
    total_general = int(bbox_df["一般牙數量(v=0)"].sum()) if not bbox_df.empty else 0

    val_metrics = extract_pose_map(model.trainer)
    summary_df = pd.DataFrame([{
        "train張數": len(train_items),
        "val張數": len(val_items),
        "train_bbox總數": total_bbox,
        "train_目標牙總數(v=2)": total_target,
        "train_一般牙總數(v=0)": total_general,
        **val_metrics,
    }])
    summary_path = work_dir / "訓練摘要.xlsx"
    with pd.ExcelWriter(summary_path, engine="openpyxl") as writer:
        summary_df.to_excel(writer, sheet_name="訓練摘要", index=False)
        bbox_df.to_excel(writer, sheet_name="train每張bbox數量", index=False)

    print(f"📍 訓練摘要：{summary_path}")
    print(f"   train bbox總數={total_bbox}（目標牙{total_target} / 一般牙{total_general}），"
          f"逐張明細見Excel的「train每張bbox數量」工作表")
    if val_metrics:
        print(f"   {val_metrics}")
        print(f"   ⚠️ 這是val集(約{len(val_items)}張)上的表現，只能看訓練有沒有基本學起來，")
        print(f"      不是正式要報告的指標——正式指標要看這個模型對33張test圖的推論結果(2c)。")
    else:
        print(f"   ⚠️ 讀不到val集Box/Pose mAP，這次只成功寫入train bbox統計。")
